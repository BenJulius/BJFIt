...
export function getCharacterProgress(userId, characterId) {
  ...
  const rawEquipped = Array.isArray(progress.equipped) && progress.equipped.length ? progress.equipped : freeItems.slice(0, 1);
  const equipped = sanitizeEquipped(characterId, rawEquipped);
  return {
...
function normalizedSlot(slot) {
  if (slot === "hat" || slot === "crown") return "head";
  return slot;
}

function sanitizeEquipped(characterId, equippedIds = []) {
  const slotToItem = new Map();
  for (const itemId of equippedIds) {
    const item = findItemForEquipped(characterId, itemId);
    if (!item) continue;
    const slots = (item.equips || []).map(normalizedSlot);
    const hasConflict = slots.some((slot) => slotToItem.has(slot));
    if (hasConflict) continue;
    slots.forEach((slot) => slotToItem.set(slot, itemId));
  }
  return [...new Set(slotToItem.values())];
}

function mergeEquippedBySlot(characterId, existing, item) {
  const removeSlots = new Set((item.equips || []).map(normalizedSlot));
  const kept = existing.filter((itemId) => {
    const existingItem = findItemForEquipped(characterId, itemId);
    return !existingItem || !(existingItem.equips || []).some((slot) => removeSlots.has(normalizedSlot(slot)));
  });
  return sanitizeEquipped(characterId, [...kept, item.id]);
}
...
export function awardAdTokens(userId, amount = 3) {
  const safeAmount = Math.max(1, Number(amount) || 1);
  const state = getCharacterState(userId);
  const nextState = {
    ...state,
    tokens: (state.tokens || 0) + safeAmount,
  };
  saveCharacterState(userId, nextState);
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("character-reward-earned", {
